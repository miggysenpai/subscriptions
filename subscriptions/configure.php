...
$edit = Input::get('edit');
$o = false;
$levels = $db->query("SELECT * FROM plg_sub_plans WHERE disabled = 0 ORDER BY ordering")->results();
if(is_numeric($edit)){
  $thisPlanQ = $db->query("SELECT * FROM plg_sub_plans WHERE id = ? AND disabled = 0",[$edit]);
  $thisPlanC = $thisPlanQ->count();
  if($thisPlanC < 1){
    Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=Plan+does+not+exist');
  }else{
...
$db->update('plg_sub_settings',1,$fields);
 Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=Saved');
}

 if(!empty($_POST['disableThis'])){
   $da = Input::get('disableThis');
   if(is_numeric($da)){
     $db->update('plg_sub_plans',$da,['disabled'=>1]);
     Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=Plan+deleted');
   }
 }
...
//PLAN PRICING OPTION
 if(!empty($_POST['plugin_cost'])){
   ...
   $fields = array(
     ...
     'stripe_price_id'=>$price_create->id,
     'descrip'=>Input::get('descrip'),
   );
   $db->insert('plg_sub_cost',$fields);
   Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=Pricing+option+added');
 }

 if(!empty($_POST['plugin_subscriptions'])){

   $perms = Input::get('perm');
   if($perms == ''){Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=You+must+add+a+permission+level&edit='.$edit);}
   $ordering = Input::get('ordering');
   if($ordering == ''){
     $grabQ = $db->query("SELECT ordering FROM plg_sub_plans ORDER BY ordering desc");
     $grabC = $grabQ->count();
     if($grabC > 0){
        $grab = $grabQ->first();
   ...
   if(!$e){
   $db->insert('plg_sub_plans',$fields);
   Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=Level+created');
 }else{
   $db->update('plg_sub_plans',$edit,$fields);
   Redirect::to('admin.php?view=plugins_config&plugin=subscriptions&err=Level+updated');
 }
  ...
  <div class="row"> 
    ...
    <div class="col-12">
      ...
      ?>
          <div class="jumbotron">
              ...
              <br />
              Create a webhook to <span class="alert-link">https://yourdomain.com/usersc/plugins/subscriptions/StripeWebhook.php?webhook=webhook</span>
              <br />
      ...
      <?php $subset = $db->query("SELECT * FROM plg_sub_settings")->first(); ?>
      <form class="" action="" method="post">
        <input type="hidden" name="csrf" value="<?=$token?>" />
      <div class="row">
        <div class="col-5 form-group">
          <label for="">Currency Symbol (1 character)</label>
          <input type="text" name="sym" value="<?=$subset->sym?>" size="1">
        </div>
        <div class="col-5 form-group">
          <label for="">Currency Code (3 letters)</label>
          <input type="text" name="cur" value="<?=$subset->cur?>" size="3">
        </div>
        <div class="col-5 form-group">
          ...
          <select class="" name="payments">
            <option value="0" <?php if($subset->payments == 0){echo "selected='selected'";}?>>No</option>
            <option value="1" <?php if($subset->payments == 1){echo "selected='selected'";}?>>Yes</option>
          </select>
        </div><br>
        <div class="col-2 form-group">
          <input type="submit" name="subset" value="Save Global Settings" class="btn-primary">
        </div>
      </div>
    </form>
    </div>
         ...
         <div class="card-deck mb-3 ">
            ...
            <div class="container">
          <form class="" action="<?php if($e){ echo "admin.php?view=plugins_config&plugin=subscriptions&edit=$edit";}?>" method="post" > 
            <input type="hidden" name="csrf" value="<?=$token?>" />
            <br />
            ...
            <div class="form-group">
              <label  for="icon">Icon Filename (Just the filename located in usersc/subscriptions/icons)</label>
            ...
            <div class="form-group">
              <label  for="script">Optional - If you want to run a certain script when this option is chosen, please put it in
                usersc/plugins/subscriptions/scripts and put the name here. There is an example in that folder.</label>
            ...
            <div class="form-group">
              <?php if($e){ ?>
                <input type="submit" name="plugin_subscriptions" value="Update Level" class="btn btn-primary">
              <?php }else{ ?>
                <input type="submit" name="plugin_subscriptions" value="Add Level" class="btn btn-primary">
              <?php } ?>
            </div></div></div>
            </div>
          </form>
    ...
    <h6 class="border-bottom border-gray  mb-0">Price Settings</h6>
        <table class="table table-striped">
          ...
          <tbody>
            ...
            foreach($levels as $l){
              $n = $l->plan_name;
               $costs = $db->query("SELECT * FROM plg_sub_cost WHERE plan = ? ORDER BY days",[$l->id])->results();
               foreach($costs as $c){ ?>
                 <tr>
                   <td><?=$n?></td>
                   <td><?=$c->descrip?></td>
                   ...
                   <td><?=$c->stripe_price_id?></td>
                   <td><?=$c->cost?></td>
                   <td><button type="button" onclick="window.location.href = '<?=$us_url_root?>usersc/plugins/subscriptions/paymentOption.php?edit=<?=$c->id?>';" name="button" class="btn btn-success">Edit</button></td>
                 </tr>
     ...
     <table class="table table-striped">
          ...
          <tbody>
            ...
            <tr>
                ...
                <td><button type="button" onclick="window.location.href = '<?=$us_url_root?>usersc/plugins/subscriptions/stripeOptions.php';" name="button" class="btn btn-success">Edit</button></td>
            </tr>
          </tbody>
        </table>
        <br />
        
    <h6 class="border-bottom border-gray  mb-0">Stripe Coupons</h6>
     <table class="table table-striped">
          <thead>
            <tr>
              <th>On/Off</th><th>Edit</th>
          </thead>
          <tbody>
            <?php
                if($keys->stripe_coupons == true){
                    $stripe_coupons = "On";
                } else {$stripe_coupons = "Off";}
            ?>
            <tr>
                <td><?=$stripe_coupons?></td>
                <td><button type="button" onclick="window.location.href = '<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=couponsOnOff';" name="button" class="btn btn-success">Edit</button></td>
            </tr>
          </tbody>
        </table> 
    ...
    <div class="row a-css">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=subscriptions">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=products">
                <div class="d-flex align-items-center">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=customers">
                <div class="d-flex align-items-center">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=transactions">
                <div class="d-flex align-items-center">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=payouts">
                <div class="d-flex align-items-center">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=refunds">
                <div class="d-flex align-items-center">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=coupons">
                <div class="d-flex align-items-center">
        ...
        <div class="col-md-4 my-3 rounded">
            <a href="<?=$us_url_root?>usersc/plugins/subscriptions/stripeSettings.php?page=invoices">
                <div class="d-flex align-items-center">
