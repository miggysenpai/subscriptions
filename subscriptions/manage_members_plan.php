...
$u = $uiQ->first();
$plans = $db->query("SELECT * FROM plg_sub_plans WHERE disabled = 0 ORDER BY ordering")->results();
...
$costSel = false;
if($ps){
	$costs = $db->query("SELECT * FROM plg_sub_cost WHERE plan = ? AND disabled = 0 ORDER BY days",[$pl])->results();

	$selected = $db->query("SELECT * FROM plg_sub_plans WHERE id = ?",[$pl])->first();

	$sel = true;
	$cst = Input::get('cst');
	if(is_numeric($cst)){
		$costSel = true;
		$costPlan = $db->query("SELECT * FROM plg_sub_cost WHERE id = ?",[$cst])->first();
		if($costPlan->plan != $pl){
	...
	<div class="container">
		<div class="row">
			<div class="col-sm-12">
				...
				<strong>Current Plan:</strong> <?=echoPlanName($u->plg_sub_level);?><br>
				<strong>Exp. Date:</strong> <?php if($u->plg_sub_exp < $date){
					$e = true; ?>
					<font color="red">
					<?php }else{
						$e = false;
					...
					<font color="black"><br>
						<?php if($sel && $costSel){
							...
							<form class="" action="manage_members_plan.php" method="get">
								...
								<?php }else{
									...
									if($u->plg_sub_level > 0 && $u->plg_sub_cost > 0){
									$oldPlan = $db->query("SELECT * FROM plg_sub_cost WHERE id = ?",[$u->plg_sub_cost])->first();
									...
									$ends = date_create($u->plg_sub_exp);
									$interval = date_diff($today, $ends);
									$interval = $interval->format('%a');
									$credit = number_format($interval * $oldcpd,2,".","");
									echo "<p>Since the existing plan has not expired, you have a credit of <font color='red'>$subset->sym $credit</font> for your $interval unused day(s)<br>at the cost of $oldcpd per day.</p>";
									if($credit > $costPlan->cost){
										...
										echo "The billable amount for the current period is: <font color='red'>$subset->sym ";
								...
								if($e){
									...
									$newdate = new DateTime($u->plg_sub_exp);
								}
								$newdate->add(new DateInterval('P'.$costPlan->days.'D'));
								?>
								<input type="date" name="expiration" value="<?=$newdate->format('Y-m-d')?>">
								<br>
								<strong>To Be Collected:<font color="red"></strong> <?=$subset->sym?>
									<input type="number" name="billed" min="0" step=".01" value="<?=number_format($billed,2,".","");?>">
									</font><br>
								<strong>The user should get a credit of <font color="red"><?=$subset->sym?></font><input type="number" min="0" step=".01" name="cred" value="<?=number_format($credit,2,".","");?>"> back to their original payment method.
							...
							if(!empty($_GET['submitFinal'])){
								...
								$fields = array(
									...
									'plg_sub_level'=>$pl,
									'plg_sub_cred'=>$cred,
									'plg_sub_cost'=>$costPlan->id,
								);
								$db->update('users',$id,$fields);
								if($cred > 0){
									logger($user->data()->id,"Subscriptions","$u->username($u->id) credited $cred for plan change");
									// $creditAccount = $creditAccount + $u->plg_sub_cred;
								}
								logger($user->data()->id,"Subscriptions","$whatHappened for $u->username($u->id) in back office. Billed $subset->sym $billed");
								if(file_exists($abs_us_root.$us_url_root.'usersc/plugins/subscriptions/scripts/'.$selected->script)){
									include $abs_us_root.$us_url_root.'usersc/plugins/subscriptions/scripts/'.$selected->script;
								}
								changeOfPlans($u->plg_sub_level,$selected->id,$u->id);
						...
						<?php if($sel && !$costSel){?>
							<form class="" action="manage_members_plan.php" method="get">
								Subscription level chosen. Now choose a plan.<br>
								<input type="hidden" name="userid" value="<?=$id?>">
								<input type="hidden" name="method" value="<?=$method?>">
								<input type="hidden" name="ps" value="<?=$pl?>">
								<select class="" name="cst">
									...
									<?php foreach($costs as $p){ ?>
										<option value="<?=$p->id?>"><?=$p->descrip?> - <?=$p->days?> days - <?=$subset->sym?><?=$p->cost?></option>
									<?php 	} ?>
								</select>
