...
$edit = Input::get('edit');
$planQ = $db->query("SELECT * FROM plg_sub_cost WHERE id = ?",[$edit]);
$planC = $planQ->count();
if($planC < 1){
	Redirect::to($us_url_root."users/admin.php?view=plugins_config&plugin=subscriptions&err=Not+found");
}
$plan = $planQ->first();
...
$stripe = new \Stripe\StripeClient($keys->stripe_s);
	...
	if(!empty($_POST['days'])){
		$fields = array(
			'cost'=>Input::get('cost'),
			'days'=>Input::get('days'),
			'stripe_price_id'=>Input::get('price'),
			'descrip'=>Input::get('descrip'),
			'disabled'=>Input::get('disabled'),
		);
		$db->update('plg_sub_cost',$edit,$fields);
		Redirect::to($us_url_root."users/admin.php?view=plugins_config&plugin=subscriptions&err=Pricing+option+updated");
	}
	
    if(Input::get('delete') == "delete"){
        $edit = Input::get('edit');
        $db->deleteById('plg_sub_cost', $edit);
        $stripe->prices->update($plan->stripe_price_id, ['active' => 'false']);
        $stripe_product = $stripe->prices->retrieve($plan->stripe_price_id,[]);
        $stripe->products->update($stripe_product->product,['active' => 'false']);
        Redirect::to($us_url_root."users/admin.php?view=plugins_config&plugin=subscriptions&err=Poduct+deleted");
    }
    
    if(Input::get('delete') == "warning"){?>
    <div class="jumbotron">
      <h1 class="display-4">BIG WARNINGGGGG!</h1>
      <p class="lead">Deleting will only happen in YOUR  database. It will get archived in stripe and no more subscriptions will be able to be made with this product.</p>
                    <br />
			...
			<div class="col-sm-12">
				...
				<form class="" action="" method="post">
					...
					<div class="form-group">
						...
						<input class="form-control" type="text" name="price" value="<?=$plan->stripe_price_id?>"  placeholder="price_xxx" required disabled>
					</div>
